...
class HealthBar extends PositionComponent with HasGameRef<FocusGame> {
  ...
  HealthBar()
    : super(position: Vector2(40, safeAreaTop), size: Vector2(180, 28));

  @override
  void render(Canvas canvas) {
    final player = gameRef.player;
    final healthPercent =
        (player.maxHealth > 0 ? (player.currentHealth / player.maxHealth) : 0.0)
            .clamp(0.0, 1.0);
    ...
    canvas.drawRect(
      Rect.fromLTWH(px, px, size.x - px * 2, size.y - px * 2),
      innerBg,
    );
    ...
    // Sağ alt köşe - koyu
    canvas.drawRect(
      Rect.fromLTWH(size.x - px, size.y - px, px, px),
      cornerDark,
    );
